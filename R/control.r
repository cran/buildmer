NSENAMES <- c('control','weights','offset','AR.start','subset')

#' Sets control options for buildmer
#' 
#' \code{buildmerControl} provides all the knobs and levers that can be manipulated during the buildmer fitting and \code{summary}/\code{anova} process. Some of these are part of buildmer's core functionality---for instance, \code{crit} allows to specify different elimination criteria, a core buildmer feature---whereas some are only meant for internal usage, e.g. \code{I_KNOW_WHAT_I_AM_DOING} is used to turn off the PQL safeguards in \code{\link{buildbam}}/\code{\link{buildgam}}, which you really should only do if you have a very good reason to believe that the PQL check is being triggered erroneously for your problem.
#' 
#' @details
...
#' All functions in the \code{buildmer} package are aware of the distinction between (f)REML and ML, and know to correct chi-square \emph{p}-values using the Stram & Lee (1994) correction when comparing nlme/lme4-style models differing only in random effects (see Pinheiro & Bates 2000).
...
#' @param grad.tol Tolerance for declaring gradient convergence.
#' @param hess.tol Tolerance for declaring Hessian convergence.
...
#' @param can.use.reml Internal option specifying whether the fitting engine should distinguish between fixed-effects and random-effects model comparisons. Do not set this option yourself unless you are programming a new fitting function for \code{\link{buildcustom}}.
#' @param force.reml Internal option specifying whether, if not differentiating between fixed-effects and random-effects model comparisons, these comparisons should be based on ML or on REML (if possible). Do not set this option yourself unless you are programming a new fitting function for \code{\link{buildcustom}}. Enabling this option only makes sense for criteria that do not compare likelihoods, in which case this is an optimization; it is applied automatically for the 'deviance-explained' criterion.
...
buildmerControl <- function(
	...
	quickstart=0,
	singular.ok=FALSE,
	grad.tol=quote(formals(buildmer::converged)$grad.tol), #these need buildmer:: namespacing to not break packages that import buildmer --- those do not transitively inherit the buildmer environment
	hess.tol=quote(formals(buildmer::converged)$hess.tol),
	dep=NULL,
	REML=NA,
	can.use.reml=TRUE,
	force.reml=FALSE,
